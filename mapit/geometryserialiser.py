...
"""
    ...
    def __collect_polygons(self, area):
        ...
        if len(all_polygons) > 1:
            all_polygons = all_polygons.aggregate(collected=Collect('polygon'))['collected']
    ...
    # output self.areas as a single geojson feature
    def geojson_feature(self):
        content_type = 'application/json'
        processed_areas = self.__process_polygons()
        if len(processed_areas) > 1:
            raise ValueError("Can only use geojson_feature for a single area, use geojson() instead.")

        polys, area = processed_areas[0]
        return (json.dumps(self.area_as_geojson_feature(area, polys)), content_type)
